...
class RecSysGNN(nn.Module):
  def __init__(
    ...
    assert (model == 'LightGCN' or model == 'tempLGCN'), 'Model must be LightGCN or tempLGCN'
    self.model = model
    ...
    if self.model == 'LightGCN':
      self.convs = nn.ModuleList(
        LGCN() for _ in range(num_layers)
      )
    else:
      self.convs = nn.ModuleList(
  ...
  def forward(self, edge_index, edge_attrs, src, dest):
